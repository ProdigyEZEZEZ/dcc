...
* MacOS
	...
	```bash
	bash <(curl -s https://raw.githubusercontent.com/ProdigyEZEZEZ/dcc/master/install_scripts/macos_install.sh)
	```
    ...
    ```bash
	sudo curl -L https://github.com/ProdigyEZEZEZ/dcc/releases/latest/download/dcc -o /usr/local/bin/dcc
	sudo chmod o+rx  /usr/local/bin/dcc
    ```
